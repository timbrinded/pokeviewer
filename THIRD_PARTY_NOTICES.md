...
The planned content pack derives sprites from the Pokémon Yellow sprite set
exposed by the [PokeAPI sprites repository][sprites]. Those files are not
covered by Pokeviewer's MIT license. Converting a sprite to a monochrome
firmware format does not remove the underlying third-party rights.
