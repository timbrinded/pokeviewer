...
4. Require `Required release tests` and
   `Reproducible v1.1.0 candidate` to pass.
5. Download the retained
   `pokeviewer-v1.1.0-candidate-COMMIT` workflow artifact.
...
`pokeviewer-v1.1.0.tar.gz` contains only:
...
version other than `1.1.0`. It also cross-checks the board, targets, flash
...
scripts/verify-release-candidate.sh \
  /tmp/pokeviewer-candidate/pokeviewer-v1.1.0.tar.gz
```
